...
export default function OfficeCleaningPage({ params: { locale } }: OfficeCleaningPageProps) {
  const t = useTranslations('services.office');
  const tPage = useTranslations('servicePages.office');
  const tCommon = useTranslations('servicePages.common');
  const tPricing = useTranslations('pricing');

  const serviceImage = 'https://images.unsplash.com/photo-1497366216548-37526070297c?w=800&h=600&fit=crop';
          ...
          <div className="grid lg:grid-cols-2 gap-12 items-center">
            ...
            >
              ...
              <Link href={`/${locale}/booking`}>
                <Button size="lg">{tPage('getQuote')}</Button>
              </Link>
            </motion.div>
        ...
        <Container>
          ...
          <div className="grid md:grid-cols-3 gap-8 max-w-5xl mx-auto">
            {['basic', 'standard', 'premium'].map((plan, index) => {
              const price = plan === 'basic' ? 70 : plan === 'standard' ? 110 : 150;
          ...
          <div className="grid lg:grid-cols-2 gap-12 items-center">
            <div>
              ...
              <ul className="space-y-4">
                {['0', '1', '2', '3', '4', '5'].map((idx, index) => (
                  <li key={idx} className="flex items-start gap-3">
                    <FaCheckCircle className="text-primary-600 mt-1 flex-shrink-0" />
                    <span className="text-gray-700">{tPage(`benefits.${idx}`)}</span>
                  </li>
                ))}
              </ul>
            </div>
            <div className="relative h-[400px] rounded-2xl overflow-hidden shadow-xl">
              <Image
                src="https://images.unsplash.com/photo-1497366754035-f200968a6e72?w=1000&h=800&fit=crop"
